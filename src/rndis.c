...
int rndis_send_init(libusb_device_handle *dev_handle, unsigned char *buf, int buflen) {
	int r = 0;
	r = libusb_control_transfer(dev_handle, USB_TYPE_CLASS | USB_RECIP_INTERFACE,
		USB_CDC_SEND_ENCAPSULATED_COMMAND, 0, 0, buf, buflen, RNDIS_CONTROL_TIMEOUT_MS);
	if (r < 0) {
		return r;
	}

	r = libusb_control_transfer(dev_handle, USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE,
		USB_CDC_GET_ENCAPSULATED_RESPONSE, 0, 0, buf, CONTROL_BUFFER_SIZE, RNDIS_CONTROL_TIMEOUT_MS);
	if (r < 0) {
		return r;
	}
